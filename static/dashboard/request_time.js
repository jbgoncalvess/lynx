...
document.addEventListener('DOMContentLoaded', () => {
    const request_time = document.getElementById('request_time_data');
    const data = JSON.parse(request_time.textContent);
    const ctx = document.getElementById('request_time').getContext('2d');
    // Configurações do gráfico de barras
    const request_time_graph = new Chart(ctx, {
        type: 'bar',
        data: {
            labels: data.container_names, // Exemplo de meses
            datasets: [{
                label: 'Tempo completo da requisição',
                data: data.rt,  // Exemplo de dados
